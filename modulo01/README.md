...
## 💈 Projeto em Destaque: Barber Shop System (Interface Gráfica)

Este projeto foi desenvolvido para facilitar o gerenciamento de serviços e o agendamento de horários em uma barbearia tradicional. Ele evoluiu de scripts em linha de comando para uma **Interface Gráfica (GUI)** robusta e moderna utilizando a biblioteca `tkinter`.



### 🚀 Funcionalidades Principais
1. **Visualização Dinâmica de Serviços:** Exibição elegante dos serviços disponíveis, preços, duração e vagas em tempo real.
2. **Agendamento Inteligente:** Popup interativo para seleção de barbeiros disponíveis e baixa automática no estoque de vagas.
3. **Painel de Configuração/Gerenciamento:** Interface administrativa para editar preços, alterar descrições, adicionar funcionários e gerenciar vagas.
4. **Controle de Estoque e Disponibilidade:** Monitoramento visual de vagas com alertas automáticos e desativação de botões para serviços esgotados.
5. **Informações Gerais e Contatos:** Telas dedicadas para localização, canais de atendimento e horários de funcionamento.

### 🛠️ Tecnologias e Estruturas Utilizadas
* **Python 3:** Linguagem base do sistema.
* **Tkinter & TTK:** Criação da interface gráfica, estilização de botões, frames estruturados e caixas de seleção dinâmica (`Combobox`).
* **Estrutura de Dados Multidimensional (Listas e Dicionários):** Armazenamento estruturado e manipulação direta do modelo de dados dos serviços.
* **Paradigma de Orientação a Objetos (POO):** Código modular organizado em torno da classe principal `BarbeariaApp`.
* **Programação Orientada a Eventos:** Uso de `lambda` e `bind` para capturar ações do usuário em tempo real.

---

## 👥 Participantes do Grupo (Destaque)

### 🌆 Tarde (Segunda e Terça)
O desenvolvimento e aprimoramento deste sistema de barbearia gráfica foi encabeçado por:

* **✂️ Barbearia:** Gabriel Carvalho, Nicolas, Kauan, Felipe, Gustavo, Victor

*(Para a lista completa de participantes e outros temas do projeto, consulte as seções inferiores deste documento).*

---

## 💻 Visualizando o Código Principal (GUI)

Abaixo está a implementação completa em Python utilizando a biblioteca **Tkinter**, que conta com uma paleta de cores temática inspirada nas barbearias clássicas (*dark blue*, *gold* e *grafite escuro*):
